...
int calcValues(void* virtual_address, int byte_count, TIEdata* tie_data) ;
...
int main(int argc, char** argv)
{
  ...
  // Setting up the broadcast server;
...
// print_nWords(cmd_buf,num_Bytes,2) ; //Test to see if the memory buffer has been initialized.
  ...
  while(1){
    ...
    stopAcq();   
    calcValues(total_data_buf, numBytesRead, &tie_data);
    tie_data.t = timeInMilliseconds();

    sendto(sockfd, &tie_data, sizeof(TIEdata), 0,
